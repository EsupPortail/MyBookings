...
class StatisticsRepository extends ServiceEntityRepository
{
    ...
    public function add(Statistics $entity, bool $flush = false): void
    {
    ...
    public function getStatisticByBookingId($bookingId)
    {
        return $this->createQueryBuilder('s')
            ->andWhere('s.booking like :bookingId')
            ->setParameter('bookingId', '%{"id":'.$bookingId.'}%')
            ->getQuery()
            ->getOneOrNullResult();
    }

    public function getStatisticsBefore($dateEnd)
    {
        return $this->createQueryBuilder('s')
            ->andWhere('(s.date_end < :end)')
